...
function tab = benford_extract(varargin)
    ...
    if (isempty(p))
        ...
        p.addOptional('extr','1ST',@(x)any(validatestring(x,{'1ST','2ND','3RD','F2D','F3D','L2D'})));
...
function tab = benford_extract_internal(data,extr,a,ccf)
    ...
    switch (extr)
        ...
        case 'F3D'
            [the_dgts,the_p,the_f,emp_c,emp_p,emp_f] = extract_first_digits(data,3);
    ...
    if (ccf)
        ...
        wci_m = (2 .* n .* the_p) + cv2;
        wci_den = 2 * (n + cv2);
        asd = cv2 - (1 / n) + (4 .* n .* the_p .* (1-the_p));

        the_p_lb = (wci_m - (cv .* sqrt(asd + wci_ccf)) + 1) ./ wci_den;
        the_p_ub = (wci_m + (cv .* sqrt(asd - wci_ccf)) + 1) ./ wci_den;
